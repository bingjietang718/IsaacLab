...
from .disassembly_env_cfg import DisassemblyEnvCfg, OBS_DIM_CFG, STATE_DIM_CFG
...
class DisassemblyEnv(DirectRLEnv):
    ...
    def _load_assembly_info(self):
        ...
        in_file = open(self.cfg_task.plug_grasp_json, "r")
    ...
    def _randomize_gripper_pose(self, sim_steps, env_ids):
        ...
        fingertip_centered_pos_noise = \
            fingertip_centered_pos_noise @ torch.diag(torch.tensor(self.cfg_task.gripper_rand_pos_noise,
                                                                   device=self.device))
        ctrl_tgt_pos += fingertip_centered_pos_noise

        # Set target rot
        ctrl_target_fingertip_centered_euler = torch.tensor(self.cfg_task.fingertip_centered_rot_initial,
    ...
    def _save_log_traj(self):
        ...
        if len(self.log_arm_dof_pos) > self.cfg_task.num_log_traj:
            ...
            log_filename = os.path.join(os.getcwd(), self.cfg_task.data_dir, self.cfg_task.assembly_id+'_disassemble_traj.json')
