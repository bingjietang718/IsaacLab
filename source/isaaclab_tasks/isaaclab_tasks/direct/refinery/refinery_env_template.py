...
# from .refinery_env_cfg import RefineryEnvCfg, OBS_DIM_CFG, STATE_DIM_CFG
from .refinery_fr3_env_cfg import RefineryEnvCfg, OBS_DIM_CFG, STATE_DIM_CFG
...
class RefineryEnv(DirectRLEnv):
    cfg: RefineryEnvCfg

    def __init__(self, cfg: RefineryEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _init_tensors(self):
        ...
        self.plug_grasps, self.disassembly_dists, self.disassembly_directions = self._load_assembly_info()
    ...
    def _load_assembly_info(self):
        ...
        in_file = open(self.cfg_task.plug_grasp_path, "r")
        plug_grasp = np.loadtxt(in_file).reshape(1, -1)
        plug_grasps = np.repeat(plug_grasp, self.num_envs, axis=0).tolist()
        plug_grasps = torch.as_tensor(plug_grasps).to(self.device)

        in_file = open(self.cfg_task.disassembly_dist_json, "r")
        disassembly_dist_dict = json.load(in_file)
        disassembly_dist = disassembly_dist_dict[self.cfg_task.step_id+".obj"]
        disassembly_dist = torch.as_tensor(disassembly_dist).to(self.device)

        in_file = open(self.cfg_task.disassembly_direction, "r")
        disassembly_direction_dict = json.load(in_file)
        disassembly_direction = disassembly_direction_dict[self.cfg_task.step_id+".obj"]["direction"]
        disassembly_direction = torch.as_tensor(disassembly_direction).to(self.device)

        return plug_grasps, disassembly_dist, disassembly_direction
    ...
    def _load_disassembly_data(self):
        ...
        in_file = open(self.cfg_task.disassembly_path_json, "r")
    ...
    def _update_rew_buf(self, curr_successes):
        ...
        curr_eef_pos = self.fingertip_midpoint_pos - self.gripper_goal_pos # relative position instead of absolute position
        rew_dict['imitation'] = automate_algo.get_imitation_reward_from_dtw(self.eef_pos_traj, 
                                                                    curr_eef_pos, 
                                                                    self.prev_fingertip_midpoint_pos, 
